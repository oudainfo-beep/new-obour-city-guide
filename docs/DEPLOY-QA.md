...
- `qa.js` لا يرسل أي طلب شبكة ما لم يُفعَّل صراحةً (`<html data-qa-api="on">`).
- المرحلة `scripts/seo-phase73-qa-offline.mjs` (آخر السلسلة) هي **مفتاح المجتمع في البناء**:
  - بلا `QA_API` (الوضع الحالي): تحذف صندوق «أسئلة وأجوبة المجتمع» الفارغ من كل الصفحات، وqa.js/qa.css،
    ورابط «اسأل وأجب — مجتمع العبور» من قائمة «المجتمع»؛ وتجعل `/ask/` `noindex,follow` مع إشعار
    «المجتمع قيد التجهيز» وتزيلها من sitemap.xml. (منذ 2026-09-25 — كان الصندوق الفارغ يظهر في ~280 صفحة.)
  - مع `QA_API=on`: تُبقي كل شيء، تُرقّم وسم `qa.js?v=…`، وتضيف `data-qa-api="on"` إلى `<html>`.

**التفعيل عند جاهزية الخادم** (بعد توجيه `/api/qa` إليه كما في الأسفل): أضف متغير البيئة
`QA_API=on` في إعدادات بناء Cloudflare Pages (Settings › Variables and secrets — Production)، ثم أعد النشر.
لا حاجة لتعديل `qa.js` ولا أي صفحة.
